...
from .constants import DESTINATION_CHOICES, CAR_CATEGORIES, TRANSMISSION_TYPES, STEERING_TYPES, BODY_TYPES, DRIVE_TYPES, \
    ...
    BRAND_CHOICES, COLOR_CHOICES, AMENITIES_CHOICES, YEAR_CHOICES
from apps.travel.constants import PAYMENT_CHOICES, PASSENGER_CAPACITY_CHOICES, MAX_IMAGES
...
class TransferImage(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.transfer.transfer_images.count() >= MAX_IMAGES:
            raise ValueError(f'Вы первысили лимит {MAX_IMAGES} изображений')
        super(TransferImage, self).save(*args, **kwargs)
